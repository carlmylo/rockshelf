...
import { createRSPackImage, cropImageToTempPNG, isJPEGRockshelfPackImage, parseRSDATBuffer } from '../../lib.exports'
...
export const editRSPackImage = async (rsPackImagePath: FilePathLikeTypes, options: EditPackageDataOptions): Promise<FilePath> => {
  ...
  const opts = { packageName: packageName || rsDataBuffer.packageName, installationSrc: rsDataBuffer.installationSrc, installationType: rsDataBuffer.installationType }

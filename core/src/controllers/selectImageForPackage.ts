...
import { ImageFile, PKGFile, TextureFile, type PKGFileJSONRepresentation } from 'rbtools'
import { dialog } from 'electron'
import { MyObject, pathLikeToFilePath } from 'node-lib'

export interface SelectImageForPackageReturnObject {
  path: string
  dataURL: string
}
 ...
 */
export const selectImageForPackage = useHandler(async (win, _, defaultPath: string): Promise<SelectImageForPackageReturnObject | false> => {
